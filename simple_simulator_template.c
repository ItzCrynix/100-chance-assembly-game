...
#define MIN 39		// "100111"; -- MIN Rx Ry				-- Rx <- (Rx <-- Rx < Ry / Ry <-- Ry < Rx)		Format: < inst(6) | Rx(3) | Ry(3)| x >
#define MAX 40		// "101000"; -- MAX Rx Ry				-- Rx <- (Rx <-- Rx > Ry / Ry <-- Ry > Rx)		Format: < inst(6) | Rx(3) | Ry(3)| x >
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case INC: // Inc é uma instrução diferente por precisar de apenas um registrador
                    // Sexto bit (depende do op; instruções vão de 0-15) define se a operação é INC ou DEC, para economizar opcode.
					//reg[rx]++;                                  // Inc Rx ou DEC
				    selM3 = rx;
                    selM4 = 8; // 8 -> 0001 (1), valor está direto no mux
                    carry = 0;
				...
				case CMP:   // seta 3 flags: maior, menor ou igual
					//if(rx == ry)
                    // parecido com ADD, mas o resultado nao importa, seta o flag register.
				    selM3 = rx;
                    selM4 = ry;
                    carry = 0;
                    OP = CMP;
				...
				case JMP:
					...
					{ // PC = MEMORY[PC];
						...
						//PC++; falhou a condição de jmp, segue o codigo
						IncPC = 1;
					// -----------------------------
					state=STATE_FETCH;
					break;
		...
		case STATE_EXECUTE:
			...
			switch(opcode){
				...
				case MIN:
					if (FR[0] == 1) // if rx is greater than ry
						selM2 = sM4;
					else
						selM2 = rx;
					...
					break;
				case MAX:
					if (FR[1] == 1) // if rx is less than ry
						selM2 = sM4;
					else 
						selM2 = rx;
